...
//        dd($request->input("saleroom"));
    ...
    public function show(Billreceived $billreceived){
        ...
        $demphay = 0;
//        dd($billreceivedId);
        if($billreceivedId[0]->list_saleroom != null || $billreceivedId[0]->list_saleroom != ""){
            for ($i = 0; $i < strlen($billreceivedId[0]->list_saleroom); $i++ ){
                if ($billreceivedId[0]->list_saleroom[$i] == ","){
                    $demphay++;
                }
            }
//            dd($demphay);
            if ($demphay > 0){
                $arrs = explode(",",$billreceivedId[0]->list_saleroom);
            }else{
//                dd("1");
                $arrs[] = (int)$billreceivedId[0]->list_saleroom;
