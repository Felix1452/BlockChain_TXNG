...
//                                    dd("haha");
                                    ...
                                    $image = \QrCode::format('png')
                                        ->size(200)
                                        ->generate("https://5a99-183-80-215-76.ngrok-free.app/checkBlock/".(count($hoangChain->chain)-1)."/".$blockString);
...
//                            dd($hoangChain);
    ...
    public function update($request, $billreceived){
        try {
            $arrsaleroom= array();
            if ($request->input('saleroom') != null){
                for ($i= 0; $i < count($request->input('saleroom')); $i++){
                    array_push($arrsaleroom, $request->input('saleroom')[$i]);
                }
                $arrSaleroom = implode(",",$arrsaleroom);
